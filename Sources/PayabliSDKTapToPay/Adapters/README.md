...
| `prepareReader() async throws` | Right after `configure(...)` | Build the processor SDK's reader, request session token, initialize session. **Drop `self.credentials` as soon as the SDK has its own copy.** **Never accept the merchant's terms on their behalf** — throw `PayabliTTPError.termsNotAccepted` and leave the reader able to answer `areTermsAccepted()`, so a host can confirm the reason without preparing again. |
| `areTermsAccepted() async throws -> Bool` | Whenever a host asks, at any point | Ask the platform each time rather than caching, since acceptance is granted and withdrawn outside this process. **Return `true` where the platform requires no acceptance** — the contract's default throws, so an adapter that does not override it reports that nothing was in a position to answer. Throw `PayabliTTPError.readerSetupFailed(reason:)` when there is no reader to ask, so a caller can tell that from "not accepted". |
