...
public enum PayabliPayInOperation: String, CaseIterable, Identifiable, Sendable, Equatable {
...
public enum PayabliPayInAuthorizationMethod: String, CaseIterable, Identifiable, Sendable, Equatable {
    case card
...
public enum PayabliPayInResultKind: String, Sendable, Equatable {
    case storedPaymentMethod
    case transaction
}

public enum PayabliPayInStoredMethodType: String, CaseIterable, Identifiable, Codable, Sendable {
    case card
...
public enum PayabliPayInPaymentMethod: Sendable {
    ...
    public struct Card: Sendable {
        public let data: PayabliPayInCardData
...
public struct PayabliPayInRequest: Sendable {
    public let paymentDetails: PayabliPayInPaymentDetails
    ...
    public let validation: PayabliPayInValidation
...
public struct PayabliPayInRequestConfiguration: Sendable {
    public let paymentDetails: PayabliPayInPaymentDetails
    public let accountId: String?
    public let customerData: PayabliPayInCustomerData?
    public let ipAddress: String?
    ...
    public func request(
        paymentMethod: PayabliPayInPaymentMethod,
        customerData formCustomerData: PayabliPayInCustomerData? = nil,
    ...
    private func mergedCustomerData(
        _ formCustomerData: PayabliPayInCustomerData?
...
extension PayabliPayInRequestConfiguration {
    var payabliViewModelSignature: String {
        [
...
public struct PayabliPayInAuthorizedRequest: Sendable {
...
public struct PayabliPayInFailure: Codable, Sendable, Equatable {
...
public struct PayabliPayInResult: Sendable, Equatable {
    public let kind: PayabliPayInResultKind
    public let code: String
    public let reason: String?
    public let explanation: String?
    public let action: String?
    public let transaction: PayabliPayInTransaction?
    public let storedPaymentMethod: PayabliPayInStoredPaymentMethod?
    public let apiResponse: PayabliPayInAPIResponse?

    public init(apiResponse: PayabliPayInAPIResponse) {
        kind = .transaction
    ...
    public init(storedPaymentMethod: PayabliPayInStoredPaymentMethod) {
...
public struct PayabliPayInAPIResponse: Codable, Sendable, Equatable {
    public let code: String
    public let reason: String?
    public let explanation: String?
    public let action: String?
    public let data: PayabliPayInTransaction?
    ...
    public init(
        ...
        explanation: String? = nil,
        action: String? = nil,
        data: PayabliPayInTransaction? = nil,
        token: String? = nil
    ) {
        self.code = code
        self.reason = reason
    ...
    public func failure(httpStatusCode: Int? = nil) -> PayabliPayInFailure {
        PayabliPayInFailure(
            code: code,
            reason: reason,
...
public struct PayabliPayInTransaction: Codable, Sendable, Equatable {
...
public struct PayabliPayInResponseData: Codable, Sendable, Equatable {
...
extension PayabliPayInPaymentMethod {
    func validate(_ validation: PayabliPayInValidation) throws {
        switch self {
        case let .card(method):
            try PayabliPayInMethodInput.card(method.data).validate(validation)
        case let .bankAccount(method):
            try PayabliPayInMethodInput.bankAccount(method.data).validate(validation)
        case let .stored(method):
            guard !method.storedMethodId.payabliCaptureTrimmed.isEmpty else {
                throw PayabliPayInError.invalidInput("Stored payment method ID is required.")
            }
        case let .cloudDevice(method):
            guard !method.device.payabliCaptureTrimmed.isEmpty else {
                throw PayabliPayInError.invalidInput("Cloud device is required.")
            }
        case let .check(method):
            guard !method.holderName.payabliCaptureTrimmed.isEmpty else {
                throw PayabliPayInError.invalidInput("Check account holder is required.")
            }
        case .cash:
            break
        }
...
private extension PayabliPayInCardData {
    func normalizedCaptureExpiration() throws -> String {
        let digits = expiration.payabliCaptureDigitsOnly
        guard digits.count == 4 else {
            throw PayabliPayInError.invalidInput("Expiration must be in MMYY or MM/YY format.")
        }
        let monthPrefix = String(digits.prefix(2))
        guard let month = Int(monthPrefix), (1 ... 12).contains(month) else {
            throw PayabliPayInError.invalidInput("Expiration month must be between 01 and 12.")
        }
...
extension PayabliPayInCustomerData {
    ...
    mutating func payabliCaptureMerge(_ override: PayabliPayInCustomerData?) {
    ...
    mutating func payabliCaptureApply<Value>(
        _ keyPath: WritableKeyPath<PayabliPayInCustomerData, Value?>,
        _ value: Value?
    ) {
