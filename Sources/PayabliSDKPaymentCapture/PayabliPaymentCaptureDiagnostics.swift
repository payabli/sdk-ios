...
public typealias PayabliPaymentCaptureDiagnosticHandler = @Sendable (PayabliPaymentCaptureDiagnosticEntry) -> Void

public struct PayabliPaymentCaptureDiagnostics: Sendable {
    public let isEnabled: Bool
    public let handler: PayabliPaymentCaptureDiagnosticHandler?

    public init(
        isEnabled: Bool = false,
        handler: PayabliPaymentCaptureDiagnosticHandler? = nil
    ) {
        self.isEnabled = isEnabled
        self.handler = handler
    }

    public static let disabled = PayabliPaymentCaptureDiagnostics()

    public static func enabled(
        handler: @escaping PayabliPaymentCaptureDiagnosticHandler
    ) -> PayabliPaymentCaptureDiagnostics {
        PayabliPaymentCaptureDiagnostics(isEnabled: true, handler: handler)
    }
}

public struct PayabliPaymentCaptureDiagnosticEntry: Identifiable, Sendable {
...
extension PayabliPaymentCaptureDiagnostics {
    ...
    private static func redactedBodyString(_ body: Data?) -> String? {
        ...
        do {
            ...
            let data = try PaymentCaptureJSONBody.data(
                from: PaymentCaptureJSONBody.normalizingCurrencyFields(in: redacted)
            )
    ...
    private static func diagnosticMessage(for error: Error) -> String {
        if let payabliError = error as? any PayabliError {
            if let detail = payabliError.detail?.payabliCaptureTrimmed.payabliCaptureNilIfEmpty, detail != payabliError.reason {
                return "\(payabliError.reason) \(detail)"
            }
