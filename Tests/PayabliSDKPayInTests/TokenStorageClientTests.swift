...
final class TokenStorageClientTests: XCTestCase {
    func testAddMethodSerializesCardPaymentMethodRequest() async throws {
        ...
        let result = try await client.addMethod(
            ...
            paymentMethod: .card(PayabliPayInCardData(
            ...
            options: PayabliPayInOptions(
                ...
                temporary: true,
                customerData: PayabliPayInCustomerData(customerId: 4440),
                fallbackAuth: true,
    ...
    func testAddMethodSerializesACHPaymentMethodRequest() async throws {
        ...
        _ = try await client.addMethod(
            ...
            options: PayabliPayInOptions(
                achValidation: true,
                vendorData: PayabliPayInVendorData(vendorId: 7890)
            )
        )
    ...
    func testInvalidCardDoesNotReachTransport() async throws {
        ...
        } catch PayabliPayInTokenStorageError.invalidInput {
            let requests = await transport.requests
    ...
    func testMissingCardZipDoesNotReachTransport() async throws {
        ...
        do {
            ...
            XCTFail("Expected validation error")
        } catch let PayabliPayInTokenStorageError.invalidInput(message) {
    ...
    func testCardLengthLimitsDoNotReachTransport() async throws {
        let scenarios: [(PayabliPayInCardData, String)] = [
    ...
    func testDeclinedPaymentMethodThrowsDomainError() async throws {
        ...
        } catch let PayabliPayInTokenStorageError.saveFailed(failure) {
    ...
    func testACHSecCodeIsHiddenAndSubmittedFromHiddenValues() async throws {
        ...
        XCTAssertFalse(viewModel.activeFields.contains(PayabliPayInField.secCode))
...
private func assertCardPaymentMethodResult(_ result: PayabliPayInStoredPaymentMethod) {
...
private final class DiagnosticSink: @unchecked Sendable {
    ...
    private var captured: [PayabliPayInDiagnosticEntry] = []

    func append(_ entry: PayabliPayInDiagnosticEntry) {
        lock.lock()
        defer { lock.unlock() }
        captured.append(entry)
    }

    func entries() -> [PayabliPayInDiagnosticEntry] {
        lock.lock()
        defer { lock.unlock() }
        return captured
    }
