...
final class StoredMethodRoundTripTests: XCTestCase {
    func testACardStoredThroughTheFormIsChargedAsACard() async throws {
        ...
        let component = PayabliPayIn(entryPoint: "entry", environment: .sandbox, transport: transport)
    ...
    func testABankAccountStoredThroughTheFormIsChargedAsABankAccount() async throws {
        ...
        let viewModel = PayabliPayInViewModel(
            component: component,
            configuration: PayabliPayInFormConfiguration(allowedMethods: [.bankAccount], defaultMethod: .bankAccount)
        )
    ...
    private func chargedPaymentMethod(
        _ stored: PayabliPayInStoredPaymentMethod,
        on component: PayabliPayIn,
        transport: RoutedTransport
    ) async throws -> [String: Any] {
        _ = try await component.capture(PayabliPayInRequest(
            paymentDetails: PayabliPayInPaymentDetails(totalAmount: 1),
