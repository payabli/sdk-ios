...
final class PaymentCaptureClientTests: XCTestCase {
    func testCaptureSerializesCardTransactionWithBearerAuthorization() async throws {
        ...
        let client = PaymentCaptureClient(
            transport: transport,
    ...
    func testAuthorizeRejectsACHBeforeTransport() async throws {
        ...
        do {
            _ = try await client.authorize(
                ...
                request: PayabliPaymentCaptureRequest(
                    ...
                    paymentMethod: .ach(PayabliPaymentCaptureACHMethod(data: PayabliACHPaymentMethodData(
        ...
        } catch let PayabliPaymentCaptureError.invalidInput(message) {
            XCTAssertEqual(message, "Only card transactions can be authorized.")
    ...
    func testCaptureAuthorizedSerializesPathAndPaymentDetailsOnly() async throws {
        ...
        _ = try await client.captureAuthorized(PayabliPaymentCaptureAuthorizedRequest(
    ...
    func testCaptureAuthorizedEscapesPathSeparatorsInTransactionId() async throws {
        ...
        _ = try await client.captureAuthorized(PayabliPaymentCaptureAuthorizedRequest(
            transId: "auth/id 1",
            paymentDetails: PayabliPaymentCapturePaymentDetails(totalAmount: 50)
        ))
    ...
    func testDeclineEnvelopeThrowsTransactionFailureInsteadOfGenericHTTPError() async throws {
        ...
        } catch let PayabliPaymentCaptureError.transactionFailed(failure) {
    ...
    func testMissingAccessTokenDoesNotReachTransport() async throws {
        ...
        } catch PayabliPaymentCaptureError.missingAccessToken {
            let requests = await transport.requests
            XCTAssertTrue(requests.isEmpty)
        } catch {
            XCTFail("Wrong error: \(error)")
    ...
    func testDiagnosticsRedactsBearerAndSensitivePaymentFields() async throws {
        ...
        let diagnostics = PayabliPaymentCaptureDiagnostics.enabled { entry in
    ...
    private func cardRequest(
        ...
        PayabliPaymentCaptureRequest(
            ...
            paymentMethod: .card(PayabliPaymentCaptureCardMethod(
...
private final class LockedDiagnosticEntry: @unchecked Sendable {
    ...
    private var entry: PayabliPaymentCaptureDiagnosticEntry?

    var value: PayabliPaymentCaptureDiagnosticEntry? {
        lock.lock()
        defer { lock.unlock() }
        return entry
    }

    func set(_ entry: PayabliPaymentCaptureDiagnosticEntry) {
        lock.lock()
        defer { lock.unlock() }
        self.entry = entry
    }
