...
@testable import PayabliSDKPaymentCapture
import PayabliSDKPaymentMethod
import SwiftUI
import XCTest

@MainActor
final class PayabliPaymentCaptureTests: XCTestCase {
    func testCaptureFacadeTracksSubmissionStateAndLastResult() async throws {
        ...
        let result = try await component.capture(PayabliPaymentCaptureRequest(
            paymentDetails: PayabliPaymentCapturePaymentDetails(totalAmount: 12.34),
            paymentMethod: .stored(PayabliPaymentCaptureStoredMethod(
                method: .card,
    ...
    func testConfigureRebuildsClientWithNewEnvironmentAndEntrypoint() async throws {
        ...
        _ = try await component.capture(PayabliPaymentCaptureRequest(
            paymentDetails: PayabliPaymentCapturePaymentDetails(totalAmount: 10),
            paymentMethod: .cash
        ))
    ...
    func testConfiguredSubmitUsesAuthorizeOperation() async throws {
        ...
        let component = PayabliPaymentCapture(
            ...
            requestConfiguration: PayabliPaymentCaptureRequestConfiguration(
                paymentDetails: PayabliPaymentCapturePaymentDetails(totalAmount: 22, currency: "USD"),
                source: "ios-sdk-test",
        ...
        let request = try XCTUnwrap(component.requestConfiguration?.request(
            paymentMethod: .card(PayabliPaymentCaptureCardMethod(data: PayabliCardPaymentMethodData(
    ...
    func testFormViewModelSubmitsConfiguredCaptureRequest() async throws {
        ...
        let component = PayabliPaymentCapture(
            ...
            operation: .capture,
            requestConfiguration: PayabliPaymentCaptureRequestConfiguration(
                paymentDetails: PayabliPaymentCapturePaymentDetails(totalAmount: 30, serviceFee: 1, currency: "USD"),
        ...
        let configuration = PayabliPaymentCaptureFormConfiguration(
            allowedMethods: [.card],
            defaultMethod: .card,
            hiddenValues: PayabliPaymentCaptureHiddenValues(methodDescription: "Configured description")
        )
        let viewModel = PayabliPaymentCaptureViewModel(
            component: component,
    ...
    func testPaymentSummaryTextDefaultsAndOverrides() {
        ...
        let defaultSummary = PayabliPaymentCapturePaymentSummaryConfiguration()
        ...
        let customSummary = PayabliPaymentCapturePaymentSummaryConfiguration(
