...
| ---------------- | ------------------------------------------------------------ | --------------------------------------------------------------------------------------- |
| **Flutter**      | `Flutter/PayabliSDKPlugin.swift`, `Flutter/payabli_sdk.dart` | MethodChannel plugin ready for Tap to Pay and tokenization; example app scaffold at `Example/PayabliFlutterDemo/`. |
| **.NET MAUI**    | `MAUI/PayabliBinding.cs`                                     | .NET 10 iOS binding library skeleton for Tap to Pay and tokenization. Regenerate via `sharpie bind` against the XCFrameworks. |
...
For tokenization-specific bridge setup, access-token handling, and sample
card/ACH calls, see
