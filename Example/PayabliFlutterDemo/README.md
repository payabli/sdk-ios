...
Minimal Flutter app wrapping Tap to Pay and card tokenization via the
...
- **Session badge** — current `PayabliTTPSessionState` color-coded in
  the app bar.
- **Card and ACH tokenization** — sample Flutter forms calling
  `PayabliTokenization.tokenizeCard(...)` and
  `PayabliTokenization.tokenizeACH(...)`, then rendering the stored-method
  response.
...
4. Edit `lib/main.dart`'s `Secrets` class to point at your partner backend
   `/payabli/token` and tokenization access-token endpoints.
...
- `PayabliTokenization.configure()` sets up the native
  `PayabliTokenization` component and wires the Dart-side
