...
missing=()
for var in VERSION S3_PUBLIC_HOST CORE_SHA256 PAYIN_SHA256 CARD_READER_CORE_SHA256; do
    if [[ -z "${!var:-}" ]]; then
        missing+=("$var")
    fi
done
...
VARS='${VERSION} ${S3_PUBLIC_HOST} ${CORE_SHA256} ${PAYIN_SHA256} ${CARD_READER_CORE_SHA256}'
